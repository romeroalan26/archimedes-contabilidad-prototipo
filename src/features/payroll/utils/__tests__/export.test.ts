...
describe("Export Utils", () => {
  ...
  describe("TSS Export", () => {
    ...
    it("should handle different contract types", () => {
      ...
      const file = generateTSSFile(
        [mockPayroll],
        [temporalEmployee, obraEmployee],
        new Date("2024-03-01")
      );
      const content = file.text();

      expect(content).toContain("temporal"); // Temporal
      expect(content).toContain("indefinido"); // Por Obra
    });
  ...
  describe("generateTSSContent", () => {
    it("should generate valid TSS content with correct format", () => {
      const content = generateTSSContent(
        [mockPayroll],
        [mockEmployee],
        new Date("2024-03-01")
      );
