...
export const validateTSSData = (
  ...
  payrolls.forEach((payroll, index) => {
    ...
    if (!employee) {
      errors.push({
        field: `payrolls[${index}].empleadoId`,
    ...
    if (!employee?.cedula) {
      errors.push({
        field: `payrolls[${index}].empleadoId`,
        message: "Cédula del empleado es requerida",
      });
    }
...
const formatTSSRow = (payroll: PayrollDetails, employee: Employee): TSSRow => {
  ...
  return {
    ...
    FECHA_INGRESO: format(new Date(employee.fechaIngreso), "dd/MM/yyyy", {
      locale: es,
    }),
...
export const generateTSSFile = (
  ...
  if (errors.length > 0) {
    throw new Error(
      `Errores de validación TSS: ${errors.map((e) => e.message).join(", ")}`
    );
  }
  ...
  const rows: TSSRow[] = payrolls.map((payroll) => {
    const employee = employees.find((e) => e.id === payroll.empleadoId);
    if (!employee)
      throw new Error(`Empleado no encontrado para nómina ${payroll.id}`);
  ...
  const worksheet = XLSX.utils.json_to_sheet([
    {
      ...
      ARS: payroll.ars,
      ISR: payroll.isr,
      Deducciones: payroll.deducciones.reduce((sum, d) => sum + d.monto, 0),
...
export const exportToPDF = (payroll: PayrollDetails): Blob => {
  ...
  doc.text(
    `Período: ${formatDate(payroll.periodo.inicio)} - ${formatDate(payroll.periodo.fin)}`,
    20,
    60
  );
  ...
  payroll.bonificaciones.forEach((bonificacion) => {
    doc.text(
      `- ${bonificacion.tipo}: ${formatCurrency(bonificacion.monto)}`,
      30,
      y
    );
    y += 10;
  ...
  // Mostrar deducciones adicionales
  doc.text("Deducciones Adicionales:", 20, y + 7);
  y += 10;
  if (payroll.deducciones && payroll.deducciones.length > 0) {
    payroll.deducciones.forEach((deduccion) => {
      doc.text(deduccion.tipo, 25, y + 7);
      doc.text(formatCurrency(deduccion.monto), 150, y + 7, { align: "right" });
      y += 10;
    });
  } else {
    doc.text("No hay deducciones adicionales", 25, y + 7);
    y += 10;
  }

  // Total
  doc.setFont("helvetica", "bold");
  doc.text("Salario Neto", 25, y + 7);
  doc.text(formatCurrency(payroll.salarioNeto), 150, y + 7, { align: "right" });
...
export const generateTSSContent = (
  payrolls: PayrollDetails[],
  employees: Employee[],
  period: Date
): string => {
  ...
  const rows = payrolls.map((payroll) => {
    ...
    const salarioBruto =
      payroll.salarioBase + calculateTotalBonificaciones(payroll);
    const deduccionesAdicionales = calculateTotalDeducciones(payroll);
    ...
    return [
      ...
      deduccionesAdicionales.toFixed(2).padStart(12, "0"),
...
export const generateExcelReport = async (
  ...
  const summaryData = payrolls.map((payroll) => {
    ...
    return {
      ...
      ARS: payroll.ars,
      ISR: payroll.isr,
      "Deducciones Adicionales": calculateTotalDeducciones(payroll),
      "Salario Neto": payroll.salarioNeto,
  ...
  // Hoja de deducciones
  const deductionsData = payrolls.flatMap((payroll) => {
    const employee = employees.find((e) => e.id === payroll.empleadoId);
    if (!employee) return [];

    return payroll.deducciones.map((deduccion) => ({
      "ID Nómina": payroll.id,
      Empleado: employee.nombre,
      Cédula: employee.cedula,
      Tipo: deduccion.tipo,
      Descripción: deduccion.descripcion,
  ...
  if (deductionsData.length > 0) {
    ...
    XLSX.utils.book_append_sheet(wb, wsDeductions, "Deducciones");
  }
...
export const generatePDFReport = async (
  ...
  doc.text(formatCurrency(payroll.salarioBase), 150, currentY + 7, {
    align: "right",
  });
  currentY += lineHeight;
  ...
  payroll.bonificaciones.forEach((bonificacion) => {
    ...
    doc.text(formatCurrency(bonificacion.monto), 150, currentY + 7, {
      align: "right",
    });
    currentY += lineHeight;
  });
  ...
  // Mostrar deducciones adicionales
  doc.text("Deducciones Adicionales:", 20, currentY + 7);
  currentY += lineHeight;
  if (payroll.deducciones && payroll.deducciones.length > 0) {
    payroll.deducciones.forEach((deduccion) => {
      doc.text(deduccion.tipo, 25, currentY + 7);
      doc.text(formatCurrency(deduccion.monto), 150, currentY + 7, {
        align: "right",
      });
      currentY += lineHeight;
    });
  } else {
    doc.text("No hay deducciones adicionales", 25, currentY + 7);
    currentY += lineHeight;
  }
  ...
  doc.text(formatCurrency(payroll.salarioNeto), 150, currentY + 7, {
    align: "right",
  });
